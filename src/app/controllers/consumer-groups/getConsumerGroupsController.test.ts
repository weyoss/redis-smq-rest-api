...
describe('getConsumerGroupsController', () => {
  it('HTTP 200 OK', async () => {
    ...
    const response1: TResponse<GetConsumerGroupsControllerResponseDTO> =
      await request.get(
        `/api/v1/namespaces/${queue.ns}/queues/${queue.name}/consumer-groups`,
      );

    expect(response1.status).toEqual(200);
    expect(response1.body?.data?.sort()).toEqual(
      [consumerGroup1, consumerGroup2].sort(),
    );
  });
});
